...
short lowercase kebab-case. See [CONTRIBUTING.md — Branch prefix reference](CONTRIBUTING.md#branch-prefix-reference)
for the canonical mapping table and footnotes.

Use the prefix that matches the work category:

- bug or inconsistency: `fix/`
- spec change: `spec/` (not used in registry-proxy)
- feature proposal: `feat/`
- task or chore: `chore/`
- documentation-only work (non-spec): `docs/`
- registry package submission (registry only): `package/`

Branch prefix categorizes work; conventional **commit** (or squash-merge) prefix
determines automated release bumps.
